...
{
    public class Model_RPT_TR093OBOFXCF
    {   
        private static string Owner = Configuration.Config.Owner;
        private static ILog Log = log4net.LogManager.GetLogger(typeof(Model_RPT_TR093OBOFXCF));

        protected Model_RPT_TR093OBOFXCF()
        {

        }
        public static ReportClass getReport(string ConnectionString, string DealNo)
        {
            Log.Info("-----Model_RPT_TR093OBOFXCF Start-----");
            Log.Info("Store Procedure : " + Model_RPT_TR093OBOFXCF.Owner + ".KKB_SP_TSS_TR093OBOFXCF");
            Log.Info("Data : DealNo=" + DealNo);

            RPT_TR093OBOFXCF report = new RPT_TR093OBOFXCF();
            Oracle o = Oracle.getInstance(ConnectionString);
            try
            {
                DataSet s = null, s2 = null;
                o.openConnection();
                o.createCommand(Model_RPT_TR093OBOFXCF.Owner + ".KKB_SP_TSS_TR093OBOFXCF", CommandType.StoredProcedure);
                o.setInputParameter("p_dealno", DealNo);
                s = o.executeReader("ref_rpt_cur", "TB_TR093OBOFXCF");
        ...
        {
            Log.Info("-----Model_RPT_TR093OBOFXCF-----");
