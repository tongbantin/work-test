...
{
    public class Model_RPT_TF048OBOBE
    {
        private static string Owner = Configuration.Config.Owner;
        private static ILog Log = log4net.LogManager.GetLogger(typeof(Model_RPT_TF048OBOBE));

        public Model_RPT_TF048OBOBE()
        {
        ...
        {
            Log.Info("-----Model_RPT_TF048OBOBE Start-----");
            Log.Info("Store Procedure : " + Model_RPT_TF048OBOBE.Owner + ".KKB_SP_TSS_TF048OBOBE");
            Log.Info("Data : Branch=" + Branch + ", Deal No=" + DealNo + "");

            RPT_TF048OBOBE report = new RPT_TF048OBOBE();            
            Oracle o = Oracle.getInstance(ConnectionString);
            try
            {
                DataSet s = null;
                o.openConnection();
                o.createCommand(Model_RPT_TF048OBOBE.Owner + ".KKB_SP_TSS_TF048OBOBE", CommandType.StoredProcedure);
                o.setInputParameter("p_branch", Branch);
                o.setInputParameter("p_deal_no", DealNo);
                s = o.executeReader("ref_rpt_cur", "TB_TF048OBOBE");
            ...
            {
                Log.Info("-----Model_RPT_TF048OBOBE Finish-----");
                o.closeConnection();
            }
            return report;
        }
